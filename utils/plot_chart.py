...
def plot_regression(pred, y, hist, sort=True):
    ...
    plt.ylabel('output')
    plt.text(0.01, 0.7, hyper_params(), fontsize=8)
    plt.text(145, 0.35, model_summary(), fontsize=4)
...
def model_summary():
    sumry = []
    model.get_cnn_model().summary(line_length=35, print_fn=lambda x: sumry.append(x))
    summary = "\n".join(sumry)
    return summary
